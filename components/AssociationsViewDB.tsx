...
const AssociationsView: React.FC<AssociationsViewProps> = () => {
    ...
    const parseAssociationData = (input: string): Partial<Association> | null => {
        ...
        // Handle both separated and continuous text formats
        let parts: string[] = [];
        
        // Try to split by tabs or commas first
        if (input.includes('\t') || input.includes(',')) {
            parts = input.split(/[\t,]/).map(p => p.trim()).filter(p => p);
        } else {
            // For continuous text, try to extract patterns
            parts = extractFromContinuousText(input);
        }
        ...
        const saudiCities = {
            ...
            'أملج': 'منطقة تبوك', 'الخبراء': 'منطقة تبوك', 'إمباري': 'منطقة تبوك',
        ...
        const categories = {
            ...
            'خدمات اجتماعية': 'اجتماعية', 'خدمات': 'اجتماعية', 'دعم': 'اجتماعية', 'صيانة': 'اجتماعية',
            'تنمية وإسكان': 'اجتماعية', 'تنمية': 'اجتماعية', 'إسكان': 'اجتماعية',
            'ثقافة وترفيه': 'ثقافة', 'رياضة': 'رياضة', 'منظمات دعم العمل الخيري': 'اجتماعية',
            'المنظمات الوسيطة': 'اجتماعية', 'العمل والتدريب': 'تدريب', 'التدريب': 'تدريب'
        };

        // Target audience mapping
        const targetAudiences = {
            'الأرامل والمطلقات': 'الأرامل والمطلقات',
            'الأرامل والمطلقات والمعلقات': 'الأرامل والمطلقات',
            'الأسر المحتاجة والبر': 'الأسر المحتاجة',
            'الفتيات والعمل التطوعي': 'الفتيات',
            'الأسر المنتجة والتمكين': 'الأسر المنتجة',
            'ريادة الأعمال النسائية': 'ريادة الأعمال',
            'الإسكان التنموي': 'الإسكان',
            'المتطوعات والعمل الخيري': 'المتطوعات',
            'المتطوعات والتمكين': 'المتطوعات',
            'الأسر والخدمات الإنسانية': 'الأسر',
            'تنمية مهارات المرأة والبر': 'النساء',
            'الفتيات والشباب': 'الفتيات والشباب',
            'الرياضيات والفتيات': 'الرياضيات',
            'الأمهات والمواليد': 'الأمهات',
            'الأطفال والأيتام': 'الأطفال والأيتام',
            'الفتيات ذوي الإعاقة': 'ذوي الإعاقة',
            'الأيتام والأسر والتمكين': 'الأيتام',
            'أهيل القيادات النسائية': 'القيادات النسائية',
            'التنمية المجتمعية والبر': 'المجتمع',
            'التوعية الصحية والنساء': 'النساء',
            'التكافل الاجتماعي والبر': 'التكافل الاجتماعي',
            'النساء والأطفال صحياً': 'النساء والأطفال',
            'التدريب والتوعية المجتمعية': 'المجتمع',
            'الأسر والتمكين الاقتصادي': 'الأسر',
            'الفتيات والناشئة': 'الفتيات',
            'الخدمات الإنسانية والبر': 'الخدمات الإنسانية',
            'أهيل الشباب والفتيات': 'الشباب والفتيات',
            'الموهوبات والمهارات': 'الموهوبات',
            'التكافل الاجتماعي والبرامج': 'التكافل الاجتماعي'
        };
    ...
    // Helper function to extract data from continuous text
    const extractFromContinuousText = (text: string): string[] => {
        const parts: string[] = [];
        
        // Extract phone numbers
        const phoneMatches = text.match(/0[1-9]\d{8}/g);
        if (phoneMatches) {
            parts.push(...phoneMatches);
        }
        ...
        // Extract "اضغط هنا"
        if (text.includes('اضغط هنا')) {
            parts.push('اضغط هنا');
        }
        
        // Extract the first part as name (before first phone number)
        const firstPhoneIndex = text.search(/0[1-9]\d{8}/);
        if (firstPhoneIndex > 0) {
            const namePart = text.substring(0, firstPhoneIndex).trim();
                    ...
                    <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-600">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div>
                                ...
                                <textarea
                                    placeholder="مثال (مفصول بفواصل):&#10;جمعية الأطفال, 0123456789, الرياض, أطفال, تم التواصل&#10;أو:&#10;0132345678, جمعية البر, جدة, إغاثة, 80%&#10;أو:&#10;https://charity.org, جمعية المساعدة, مكة, صحة, info@charity.org&#10;&#10;مثال (مفصول بـ Tab):&#10;عون لرعاية المطلقات بأملج	0547121414	أملج	الخدمات الاجتماعية	دعم الدخل وصيانته	اضغط هنا	الأرامل والمطلقات&#10;&#10;مثال (نص مستمر):&#10;أصالة النسائية بعفيف0534567211عفيفالتنمية والإسكانالتنمية الاجتماعية والاقتصاديةاضغط هناتمكين الأسر والنساء"
                                    value={quickAddText}
                                ...
                                <div className="mt-2 text-xs text-slate-400">
                                    💡 النظام يدعم الآن: الفواصل، Tab، والنص المستمر. يتعرف تلقائياً على: الاسم، الجوال، المدينة، التصنيف، الحالة، الإيميل، الموقع، نسبة الاستجابة، الفئة المستهدفة
                                </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-2 text-sm text-slate-300">
                                    ...
                                    <div className="bg-slate-700 p-2 rounded">
                                        <strong>مفصول بـ Tab (تنسيق Excel):</strong><br/>
                                        عون لرعاية المطلقات بأملج	0547121414	أملج	الخدمات الاجتماعية	دعم الدخل وصيانته	اضغط هنا	الأرامل والمطلقات
                                    </div>
                                    <div className="bg-slate-700 p-2 rounded">
                                        <strong>نص مستمر (بدون فواصل):</strong><br/>
                                        أصالة النسائية بعفيف0534567211عفيفالتنمية والإسكانالتنمية الاجتماعية والاقتصاديةاضغط هناتمكين الأسر والنساء
                                    </div>
                                </div>
                            </div>
